...
def clear_creation_state(level_data: dict | None = None) -> None:
    """Limpia el cuento activo cuando cambia el nivel o se reinician elecciones."""
    st.session_state.selection = empty_selection(level_data) if level_data else {}
...
def reset_choice_widget_values(level_key: str, level_data: dict, selection: dict | None = None) -> None:
    """Sincroniza los widgets de elección con una selección concreta.

    Es útil al cambiar de nivel, reiniciar o usar “Sorpréndeme”, porque
    Streamlit conserva el estado de los widgets entre reruns.
    """
    selection = selection or empty_selection(level_data)
...
def reset_all_choice_widget_values() -> None:
    """Evita que opciones libres antiguas reaparezcan al volver a un nivel."""
    for level_key in LEVEL_LABELS.keys():
        reset_choice_widget_values(level_key, cached_level(level_key))
...
def render_crear_cuento() -> None:
    ...
    with col_random:
        if st.button("🎲 Sorpréndeme", use_container_width=True):
            new_selection = random_selection(level_data)
            reset_choice_widget_values(st.session_state.level_key, level_data, new_selection)
            st.session_state.custom_selection = {}
            st.session_state.selection = new_selection
            st.session_state.draft = ""
            st.session_state.section_texts = {}
            st.session_state.final_story = ""
            st.rerun()
    with col_reset:
        if st.button("↩️ Reiniciar elecciones", use_container_width=True):
            reset_choice_widget_values(st.session_state.level_key, level_data)
            clear_creation_state(level_data)
            st.rerun()
...
def render_cuento_final() -> None:
    ...
    with col3:
        story_data = {
            ...
            "custom_selection": st.session_state.custom_selection,
